<template>
  <div class="tags-view-container" v-if="visitedViews.length>0?(visitedViews.length==1?(visitedViews[0].path=='/'?false:true):true):false">
...
export default {
  ...
  methods: {
    ...
    isActive (route) {
      if ((route.path === this.$route.path || route.name === this.$route.name) && this.$store.state.tagsView.cachedViews.length > 0) {
