<template>
  <section>
    ...
    <xt-search @search="handleSearch">
        ...
        <xt-input-number type="number" v-model.number="filters.duration" placeholder="耗时" size="small" :controls="false" class="search-box__input"/>
    ...
    <el-table :data="resLongTimeList" highlight-current-row v-loading="loading.listLoading" style="width: 100%;" stripe border>
      ...
      <el-table-column align="left" header-align="center" prop="para" label="参数" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column align="left" header-align="center" prop="duration" label="耗时" width="100" :show-overflow-tooltip="true"></el-table-column>
    ...
    <el-dialog width="95%" top="20px" :title="formData.id==0?'新增':'修改'" :visible.sync="dialog.formDataVisible" :close-on-click-modal="false" :modal-append-to-body="false">
      <el-form :model="formData" label-width="100px" :rules="formRules" ref="formData">
        ...
        <el-form-item label="耗时" prop="duration">
            <xt-input-number type="number" v-model.number="formData.duration" auto-complete="off" :maxlength="0" placeholder="请输入耗时" size="small" style="width:100%"></xt-input-number>
        </el-form-item>
      </el-form>
...
import {ResLongTimeAPI} from "../../modules";
...
export default {
  ...
  methods: {
      ...
      this.$refs.formData.validate((valid) => {
        if (valid) {
          ...
          if (params.id === 0) {
            ...
            ResLongTimeAPI.insertResLongTime(params).then(({data}) => {
              if (data.code === 1) {
          ...
          this.loading.addLoading = true;
          ResLongTimeAPI.updateResLongTime(params).then(({data}) => {
            if (data.code === 1) {
              this.$refs.formData.resetFields();
              this.getMainList();
              this.dialog.formDataVisible = false;
    ...
    deleteData(row) {
      this.$confirm("删除之后将无法恢复, 是否继续?", "提示", {
        ...
        ResLongTimeAPI.deleteResLongTime({id: row.id}).then(({data}) => {
          if (data.code === 1) {
            this.getMainList();
          }
        });
    ...
    handleAdd() {
      this.$refs.formData && this.$refs.formData.resetFields();
      this.formData = ResLongTimeAPI.init();
      this.dialog.formDataVisible = true;
    },
    ...
    getMainList() {
      ...
      ResLongTimeAPI.listResLongTime(params).then(({data, list}) => {
        this.resLongTimeList = list;
  ...
  data() {
    return {
      filters: ResLongTimeAPI.init(),
      formData: ResLongTimeAPI.init(),
      formRules: {
      },
